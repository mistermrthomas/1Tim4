...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (body.currentPlan != null) {
      ...
      if (!parsed.success) {
        res.status(400).json({
          error: 'Invalid current plan for regeneration',
          code: 'INVALID_INPUT',
        });
        return;
      }
    ...
    const message = err instanceof Error ? err.message : 'UNKNOWN';
    console.error('sermon-plan top-level failure', message);
    try {
      res.status(500).json({
        error: 'AI planning failed unexpectedly. Your notes are preserved — try again.',
        code: 'SERVER_ERROR',
      });
    } catch {
