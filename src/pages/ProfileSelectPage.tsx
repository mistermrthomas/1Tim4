...
export function ProfileSelectPage({ switching }: ProfileSelectPageProps) {
  ...
  return (
    <main className="profile-select page-content page-content--form">
      <div className="profile-select__banner">
        <HeroArt visualKey="default" alt="" />
        <div className="profile-select__banner-scrim" aria-hidden="true" />
        <div className="profile-select__banner-label">
      ...
      <article className="profile-select__card card">
        ...
        <form className="profile-select__create" onSubmit={handleCreate}>
          ...
          {error && <p className="field-hint" style={{ color: 'var(--terracotta)' }}>{error}</p>}
        </form>

        {switching && (
          <button type="button" className="btn btn-ghost" style={{ marginTop: 12 }} onClick={handleBack}>
            Cancel
          </button>
        )}
      </article>
